...
const INVALID_JSON_RETRY_PROMPT =
  "上一次输出不是合法 JSON。请只输出一个合法 JSON 对象，不要包含任何解释、注释或 Markdown 代码块。";
...
export class AiProvider {
  ...
  async completeOnce(options) {
    ...
    if (!data || typeof data !== "object" || Array.isArray(data)) {
      throw new AiProviderError("模型未返回有效 JSON", "invalid_json");
    }
    return data;
  }

  // invalid_json 时最多重试 retries 次，并追加修正提示。
  async complete(options) {
    ...
    for (let attempt = 0; attempt <= retries; attempt += 1) {
      try {
        ...
        if (error?.code !== "invalid_json" || attempt === retries) throw error;
        const base = options.messages || this.buildMessages(options);
        messages = [...base, { role: "user", content: INVALID_JSON_RETRY_PROMPT }];
      }
    }
